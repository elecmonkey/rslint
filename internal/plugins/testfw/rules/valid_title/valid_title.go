...
type Config struct {
	...
	AllowedEachSpecifiers  string
	TrimTitlePrefix        func(string) string
}
...
func titleInner(sourceFile *ast.SourceFile, n *ast.Node) (string, bool) {
	if n == nil {
		return "", false
	}
	switch n.Kind {
...
// Only run printf checks for the outer parameterized title call
// (when .each/.for(...) is invoked as (...)(title, cb)), not tagged-template factories.
func shouldValidateEachPrintf(fn *testfw.ParsedFnCall, call *ast.CallExpression, parameterizedModifiers []string) bool {
	if fn == nil || call == nil {
		return false
	}
	for _, entry := range fn.MemberEntries {
		if !slices.Contains(parameterizedModifiers, entry.Name) || entry.Node == nil {
			continue
		}
...
func eachInvalidSpecifier(title string, allowedSpecifiers string) string {
	s := strings.ReplaceAll(title, "%%", "")
	pattern := `%[^` + regexp.QuoteMeta(allowedSpecifiers) + `]`
	return regexp.MustCompile(pattern).FindString(s)
}

func emptyFunctionName(kind testfw.FnKind) string {
	if kind == testfw.FnKindDescribe {
		return "describe"
	}
	return "test"
}

func defaultTrimTitlePrefix(word string) string {
	return word
}

func NewRule(config Config) rule.Rule {
	if config.AllowedEachSpecifiers == "" {
		config.AllowedEachSpecifiers = "psdifjo#$%"
	}
	if config.TrimTitlePrefix == nil {
		config.TrimTitlePrefix = defaultTrimTitlePrefix
	}
	return rule.Rule{
		Name: config.Name,
		Run: func(ctx rule.RuleContext, options any) rule.RuleListeners {
			...
			return rule.RuleListeners{
				ast.KindCallExpression: func(node *ast.Node) {
					...
					if fn.Kind != testfw.FnKindDescribe && fn.Kind != testfw.FnKindTest {
						return
					}
					...
					title, ok := titleInner(ctx.SourceFile, arg)
					if !ok {
						if binaryExprContainsStringLit(arg) {
							return
						}
						ignored := false
						if fn.Kind == testfw.FnKindDescribe && co.ignoreTypeOfDescribeName {
							ignored = true
						}
						if fn.Kind == testfw.FnKindTest && co.ignoreTypeOfTestName {
							ignored = true
						}
					...
					if title == "" {
						ctx.ReportNode(node, rule.RuleMessage{
							Id:          "emptyTitle",
							Description: emptyFunctionName(fn.Kind) + " should not have an empty title",
							Data: map[string]string{
								"jestFunctionName": emptyFunctionName(fn.Kind),
							},
						})
						return
					}

					if shouldValidateEachPrintf(fn, call, config.ParameterizedModifiers) {
						if spec := eachInvalidSpecifier(title, config.AllowedEachSpecifiers); spec != "" {
					...
					unprefixedName := config.TrimTitlePrefix(fn.Name)
					firstTok := title
					...
					fnKey := config.TrimTitlePrefix(fn.Name)
