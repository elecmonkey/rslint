...
func isTodoTestCall(fn *testfw.ParsedFnCall) bool {
	if fn == nil || fn.Kind != testfw.FnKindTest {
		return false
	}
	return len(fn.Members) > 0 && fn.Members[len(fn.Members)-1] == "todo"
}
		...
		Run: func(ctx rule.RuleContext, options any) rule.RuleListeners {
			...
			return rule.RuleListeners{
				ast.KindCallExpression: func(node *ast.Node) {
					...
					calleeName := testfw.CalleeChainName(callExpr.Expression)
					fn := config.Parse(node, ctx)
					isTest := fn != nil && fn.Kind == testfw.FnKindTest
					isExtraBlock := calleeName != "" && slices.Contains(additionalTestBlocks, calleeName)

					if isTest || isExtraBlock {
						if isTodoTestCall(fn) || strings.HasSuffix(calleeName, ".todo") {
							return
						}
						if isTest {
